...
class _InitLoggingFromDictConfigAction(_LoggingConfigAction):
    """Records a call to LoggingUtils.init_logging_from_dict, for later replay."""

    def __init__(self, config: Dict[str, Any]):
        self._config = config

    def do_init(self) -> None:
        puma.logging.logging.Logging.init_logging_from_dict(self._config)
...
class CurrentLoggingConfiguration:
    """Records the logging initialisation that has been applied in the current process, so it can be re-applied later, or used in the log-listening process.
    ...
    def record_init_logging_from_dict_action(self, config: Dict[str, Any]) -> None:
        self._init_actions.append(_InitLoggingFromDictConfigAction(config))
    ...
    def copy(self) -> 'CurrentLoggingConfiguration':
        ret = CurrentLoggingConfiguration()
        ret._init_actions = self._init_actions.copy()
        ret._memory_handlers = self._memory_handlers.copy()
        return ret


LogListenerProcessConfiguration = CurrentLoggingConfiguration

import puma.logging.logging  # noqa: E402, I100, I202
# This import is at the end of the file, and has this 'import X' form, because
# of the circular dependency between LoggingUtils and CurrentLoggingConfiguration
